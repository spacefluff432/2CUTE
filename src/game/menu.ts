...
function menuText (
   { x = 0, y = 0 }: Partial<X2>,
   { color = '#fff', font = '16px Menu', size = 9, spacing = {} as Partial<X2> },
   ...
   navigators: {
      ...
      sidebar: new XNavigator({
         ...
         objects: [
            menuBox(
               ...
               menuText({ x: 4, y: 21 }, presets.miniStat, 'LV'),
               menuText({ x: 22, y: 21 }, presets.miniStat, () => manager.stat(SAVE).lv.toString()),
               menuText({ x: 4, y: 30 }, presets.miniStat, 'HP'),
               menuText({ x: 22, y: 30 }, presets.miniStat, () => `${SAVE.hp}/${manager.stat(SAVE).hp}`),
               menuText({ x: 4, y: 39 }, presets.miniStat, 'G'),
               menuText({ x: 22, y: 39 }, presets.miniStat, () => SAVE.g.toString())
            ),
            menuBox(
               { h: 68, w: 65, x: 16, y: 84 },
               menuSoul({ x: 9, y: 11 }, () => selection('sidebar', 'sidebarItem')),
               menuText({ x: 23, y: 11 }, {}, 'ITEM').wrapOn('tick', text => {
      ...
      sidebarStat: new XNavigator({
         ...
         objects: [
            menuBox(
               ...
               menuText({ x: 11, y: 47 }, {}, () => `LV \xa0\xa0\xa0${manager.stat(SAVE).lv}`),
               menuText({ x: 11, y: 63 }, {}, () => `HP \xa0\xa0\xa0${SAVE.hp} / ${manager.stat(SAVE).hp}`),
               menuText({ x: 11, y: 95 }, {}, () => {
               ...
               menuText({ x: 95, y: 111 }, {}, () => `NEXT: ${manager.stat(SAVE).xp}`),
               menuText({ x: 11, y: 132 }, {}, () => `WEAPON: ${SAVE.weapon}`),
               menuText({ x: 11, y: 148 }, {}, () => `ARMOR: ${SAVE.armor}`),
               menuText({ x: 11, y: 168 }, {}, () => `GOLD: ${SAVE.g}`)
            )
         ]
      })
         .on('from', atlas => {
      ...
      sidebarCell: new XNavigator({
         ...
         objects: [
            menuBox(
               ...
               menuText({ x: 19, y: 15 }, {}, "Toriel's Phone"),
               menuSoul({ x: 7, y: 31 }, () => selection('sidebarCell', 'sidebarCellPapyrus')),
               menuText({ x: 19, y: 31 }, {}, 'Papyrus and Undyne'),
               menuSoul({ x: 7, y: 47 }, () => selection('sidebarCell', '0')),
               menuText({ x: 19, y: 47 }, {}, 'Dimensional Box A'),
               menuSoul({ x: 7, y: 63 }, () => selection('sidebarCell', '1')),
               menuText({ x: 19, y: 63 }, {}, 'Dimensional Box B')
            )
         ]
      })
         .wrapOn('from', self => {
      ...
      sidebarCellBox: new XNavigator({
         ...
         objects: [
            menuBox(
               { h: 219, w: 299, x: 8, y: 8 },
               menuText({ x: 41, y: 8 }, {}, 'INVENTORY'),
               menuText({ x: 213, y: 8 }, {}, 'BOX'),
               menuText({ x: 89, y: 196 }, {}, 'Press [X] to Finish'),
               new XObject({
                  stroke: '#fff',
                  line: { width: 1 },
                  position: { x: 149, y: 29 },
         ...
         return new XNavigator({
            ...
            next: self => {
               if (self.selection() === 'save') {
                  saved = true;
                  for (const text of saveText.objects) text.fill = '#ff0';
            ...
            .on('to', () => {
               ...
               for (const text of saveText.objects) text.fill = '#fff';
               global.interact = false;
               saved = false;
            })
            .on('move', () => {
