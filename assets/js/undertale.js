...
const helper = {
    ...
    teleport(destination, origin) {
        overworld.state.room = destination;
        if (overworld.room) {
            let moved = false;
            if (origin) {
    ...
    nav: {
        show(...names) {
            for (const name of names)
                atlas.attach(name, overworld);
        },
        hide(...names) {
            for (const name of names)
                atlas.detach(name, overworld);
        },
        textStyle: {
            color: '#ffffffff',
        ...
        fixedSize(items, size) {
            return [...items, ...new Array(size - items.length).join(' ').split(' ')];
        },
        optionGrid(navigator, size, provider, extra = {}) {
            return new XItem({
                style: Object.assign({
                    columnGap: '3px',
                    display: 'grid',
                ...
                children: new Array(size)
                    ...
                    .map((x, index) => {
                    // @ts-expect-error
                    const image = GAME.soul.image.cloneNode();
                    return [
                        new XItem({
                            element: () => {
        ...
        topLevel(bounds, space, children, extra = {}) {
            return new XItem({
                style: Object.assign({
                    // menu boundaries
...
// keys
const up = new XKey('w', 'W', 'ArrowUp');
const left = new XKey('a', 'A', 'ArrowLeft');
const down = new XKey('s', 'S', 'ArrowDown');
const right = new XKey('d', 'D', 'ArrowRight');
const interact = new XKey('z', 'Z', 'Enter');
const special = new XKey('x', 'X', 'Shift');
const menu = new XKey('c', 'C', 'Control');
// player entity
...
const GAME = {
    ...
    soul: new XTexture({
        // spr_heart_1
        source: 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAABHNCSVQICAgIfAhkiAAAAGBJREFUOE9jYICC/wwM/0EYxsdFo6tjBCnEphEoAZaDAVxqGLFJwDTBDMGnhgnZFnLYeF1AjIEUu2DUAKTEgi+uscUGLI1QHIgoyRVkEyGXoCdx6rsAV+ZBtxmmjmIXAAACHRgNoVCjGAAAAABJRU5ErkJggg=='
    }),
    speed: 3,
    ...
    set interact(value) {
        ...
        if (value) {
            ...
            up.active && sprites.up.disable();
            left.active && sprites.left.disable();
            down.active && sprites.down.disable();
            right.active && sprites.right.disable();
        }
    },
    item(index, action) {
        ...
        switch (action) {
            case 0:
                dialoguer.add(`[sprite:default|sound:narrator|interval:50]\n${GAME.items[item].use}`);
                break;
            case 1:
...
const atlas = new XAtlas({
    navigators: {
        dialoguer: new XNavigator({
            ...
            item: helper.nav.topLevel({ h: 70, w: 283, x: 16, y: 160 }, { x: 0, y: 0 }, [
                new XItem({
                    style: { width: '50px', height: '50px', margin: '10px' },
                    element() {
                        const sprite = dialoguer.sprite;
                        if (sprite)
                            return (sprite.compute() || {}).image;
                    }
                }),
                new XItem({
        ...
        sidebarAddon: new XNavigator({
            item: helper.nav.topLevel({ h: 49, w: 65, x: 16, y: 26 }, { x: 4, y: 5 }, [
        ...
        sidebarStat: new XNavigator({
            ...
            item: helper.nav.topLevel({ h: 203, w: 167, x: 94, y: 26 }, { x: 11, y: 17 }, [
                ...
                new XItem({
                    element: () => `<div>ARMOR: ${isFinite(SAVE.g) ? SAVE.g : 'INF'}</div>`,
        ...
        sidebarCellBox: new XNavigator({
            ...
            from(atlas) {
                ...
                left.on('down', listener);
                right.on('down', listener);
                X.once(special, 'down', () => {
                    left.off('down', listener);
                    right.off('down', listener);
                });
            },
            prev: 'sidebarCell',
            next(atlas) {
        ...
        sidebarCellBoxAddon: new XNavigator({
            item: new XItem({
                style: Object.assign({ opacity: '0' }, helper.nav.textStyle),
                children: [
                    new XItem({
            ...
            item: new XItem({
                ...
                children: new Array(4).join(' ').split(' ').map((x, index) => {
                    ...
                    return new XItem({
                        element: () => {
                            //@ts-expect-error
                            const button = GAME.battle.buttons[GAME.battle.button[index]];
...
up.on('down', () => atlas.navigate('move', 'vertical', -1));
left.on('down', () => atlas.navigate('move', 'horizontal', -1));
down.on('down', () => atlas.navigate('move', 'vertical', 1));
right.on('down', () => atlas.navigate('move', 'horizontal', 1));
menu.on('down', () => atlas.navigate('menu'));
special.on('down', () => atlas.navigate('prev'));
interact.on('down', () => {
    if (atlas.state.navigator === null) {
        const room = overworld.room;
        if (room && !GAME.state.interact) {
...
overworld.on('trigger', ({ metadata: { key } }) => {
    typeof key === 'string' && key.startsWith('teleport:') && overworld.teleport(key.slice(9), overworld.state.room);
});
overworld.on('interact', ({ metadata: { key, dialogue } }) => {
    if (key === 'dialogue' && dialogue instanceof Array) {
        this.navigate('next', this.dialoguer);
