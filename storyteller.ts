...
class XEngine {
   animations: { down: XAnimation; left: XAnimation; right: XAnimation; up: XAnimation };
   canvas: HTMLCanvasElement;
   context?: CanvasRenderingContext2D;
   debug: boolean;
   hooks: Set<(event: 'collide' | 'interact' | 'trigger', entity: XEntity) => void> = new Set();
   inputs: { down: XInput; left: XInput; right: XInput; up: XInput };
   player: XEntity;
   queue: Set<{ event: 'collide' | 'interact' | 'trigger'; entity: XEntity }> = new Set();
   room?: XRoom;
   rooms: Map<string, XRoom>;
   scale: number = 1;
   state: { room?: string } = {};
   constructor (
      canvas: HTMLCanvasElement,
      ...
      {
         animations: {
            left: l1 = new XAnimation(),
            right: r1 = new XAnimation(),
            down: d1 = new XAnimation(),
            up: u1 = new XAnimation()
         } = {},
         debug = false,
         framerate = 30,
         inputs: { left: l2 = 'ArrowLeft', right: r2 = 'ArrowRight', down: d2 = 'ArrowDown', up: u2 = 'ArrowUp' } = {},
         rooms = {},
         state = {}
      }: {
         animations?: {
   ...
   render () {
      if (this.context) {
         ...
         if (this.room) {
            ...
            if (this.inputs.right.state || this.inputs.left.state || this.inputs.up.state || this.inputs.down.state) {
               ...
               if (colliders.size > 0) {
                  for (const collider of colliders) this.queue.add({ event: 'collide', entity: collider });
            ...
            this.context.setTransform(
               ...
               this.scale,
               (Math.min(
                  Math.max(this.player.position.x, offset.x + this.room.hitbox.x),
                  offset.x + this.room.hitbox.x + this.room.hitbox.w
               ) *
                  -1 +
                  offset.x) *
                  this.scale,
               (Math.min(
                  Math.max(this.player.position.y, offset.y + this.player.hitbox.h + this.room.hitbox.y),
                  offset.y + this.room.hitbox.y + this.room.hitbox.h
               ) +
                  offset.y) *
                  this.scale
            );
            for (const entity of [ ...this.room.visibles, this.player ].sort((a, b) => a.position.z - b.position.z)) {
               ...
               if (texture instanceof XTexture) {
                  this.context.drawImage(
                     ...
                     texture.mapping.h,
                     entity.position.x,
                     entity.position.y * -1 - texture.mapping.h,
                     texture.mapping.w,
                     texture.mapping.h
                  );
               }
      ...
      for (const entry of this.queue) for (const hook of this.hooks) hook(entry.event, entry.entity);
      this.queue.clear();
   }
   update (state: { room?: string } = {}) {
...
const X = {
   intersection ({ x = 0, y = 0, h = 0, w = 0 }: X4Arg, ...entities: XEntity[]) {
   ...
   async import (source: string): Promise<any> {
      if (X.imports.has(source)) {
         return X.imports.get(source);
      } else {
         let data: any, value: any;
         try {
            ...
            case 'XAnimation':
               if (data.attributes != null) {
                  if (typeof data.attributes !== 'object' || data.attributes instanceof Array) {
                     throw new TypeError(
                        `Invalid Resource: ${source} - If defined, property "attributes" must be of type "object"`
                     );
                  } else {
                     if (data.attributes.single == null) {
                        if (typeof data.attributes.single !== 'boolean') {
                           throw new TypeError(
                              `Invalid Resource: ${source} - If defined, property "attributes.single" must be of type "boolean"`
                           );
                        }
                     }
                     if (data.attributes.sticky != null) {
                        if (typeof data.attributes.sticky !== 'boolean') {
                           throw new TypeError(
                              `Invalid Resource: ${source} - If defined, property "attributes.sticky" must be of type "boolean"`
                           );
                        }
                     }
                  }
               ...
               if (data.default != null) {
                  if (typeof data.default !== 'number') {
                     throw new TypeError(
                        `Invalid Resource: ${source} - If defined, property "default" must be of type "number"`
                     );
                  } else if (data.default < 0) {
                     throw new RangeError(
                        `Invalid Resource: ${source} - Property "default" falls outside of the valid range [0..]`
                     );
                  }
               }
               if (data.steps != null) {
                  if (typeof data.steps !== 'number') {
                     throw new TypeError(
                        `Invalid Resource: ${source} - If defined, property "steps" must be of type "number"`
                     );
                  } else if (data.steps < 1) {
                     throw new RangeError(
                        `Invalid Resource: ${source} - Property "steps" falls outside of the valid range [1..]`
                     );
                  }
               }
               if (data.textures != null) {
                  if (typeof data.textures !== 'object' || !(data.textures instanceof Array)) {
                     throw new TypeError(
                        `Invalid Resource: ${source} - If defined, property "textures" must be of type "array"`
                     );
                  } else {
                     for (const [ index, texture ] of data.textures.entries()) {
                        if (typeof texture !== 'string') {
                           throw new TypeError(
                              `Invalid Resource: ${source} - All values in the "textures" array must be of type "string"`
                           );
                        } else {
                           ...
                           if (value instanceof XTexture) {
                              ...
                              throw new ReferenceError(
                                 `Invalid Resource: ${source} - The value "${texture}" at index ${index} in the "textures" array must refer to a texture file`
                              );
                           }
                        }
                     }
            ...
            case 'XEntity':
               if (data.animation != null) {
                  if (typeof data.animation !== 'string') {
                     throw new TypeError(
                        `Invalid Resource: ${source} - If defined, property "animation" must be of type "string"`
                     );
                  } else {
                     ...
                     if (value instanceof XAnimation) {
                        ...
                        throw new ReferenceError(
                           `Invalid Resource: ${source} - If defined, property "animation" must refer to an animation file`
                        );
                     }
                  }
               }
                  ...
                  if (typeof data.attributes !== 'object' || data.attributes instanceof Array) {
                     ...
                     if (data.attributes.collidable != null) {
                        if (typeof data.attributes.collidable !== 'boolean') {
                           throw new TypeError(
                              `Invalid Resource: ${source} - If defined, property "attributes.collidable" must be of type "boolean"`
                           );
                        }
                     }
                     if (data.attributes.interactable != null) {
                        if (typeof data.attributes.interactable !== 'boolean') {
                           throw new TypeError(
                              `Invalid Resource: ${source} - If defined, property "attributes.interactable" must be of type "boolean"`
                           );
                        }
                     }
                     if (data.attributes.triggerable != null) {
                        if (typeof data.attributes.triggerable !== 'boolean') {
                           throw new TypeError(
                              `Invalid Resource: ${source} - If defined, property "attributes.triggerable" must be of type "boolean"`
                           );
                        }
                     }
                     if (data.attributes.visible != null) {
                        if (typeof data.attributes.visible !== 'boolean') {
                           throw new TypeError(
                              `Invalid Resource: ${source} - If defined, property "attributes.visible" must be of type "boolean"`
                           );
                        }
                     }
                  }
               ...
               if (data.hitbox != null) {
                  if (typeof data.hitbox !== 'object' || data.hitbox instanceof Array) {
                     throw new TypeError(
                        `Invalid Resource: ${source} - If defined, property "hitbox" must be of type "object"`
                     );
                  } else {
                     if (data.hitbox.h != null) {
                        if (typeof data.hitbox.h !== 'number') {
                           throw new TypeError(
                              `Invalid Resource: ${source} - If defined, property "hitbox.h" must be of type "number"`
                           );
                        }
                     }
                     if (data.hitbox.w != null) {
                        if (typeof data.hitbox.w !== 'number') {
                           throw new TypeError(
                              `Invalid Resource: ${source} - If defined, property "hitbox.w" must be of type "number"`
                           );
                        }
                     }
                     if (data.hitbox.x != null) {
                        if (typeof data.hitbox.x !== 'number') {
                           throw new TypeError(
                              `Invalid Resource: ${source} - If defined, property "hitbox.x" must be of type "number"`
                           );
                        }
                     }
                     if (data.hitbox.y != null) {
                        if (typeof data.hitbox.y !== 'number') {
                           throw new TypeError(
                              `Invalid Resource: ${source} - If defined, property "hitbox.y" must be of type "number"`
                           );
                        }
                     }
                  }
               ...
               if (data.metadata != null) {
                  if (typeof data.metadata !== 'object' || data.metadata instanceof Array) {
                     throw new TypeError(
                        `Invalid Resource: ${source} - If defined, property "metadata" must be of type "object"`
                     );
                  }
               }
               if (data.position != null) {
                  if (typeof data.position !== 'object' || data.position instanceof Array) {
                     throw new TypeError(
                        `Invalid Resource: ${source} - If defined, property "position" must be of type "object"`
                     );
                  } else {
                     if (data.position.x != null) {
                        if (typeof data.position.x !== 'number') {
                           throw new TypeError(
                              `Invalid Resource: ${source} - If defined, property "position.x" must be of type "number"`
                           );
                        }
                     }
                     if (data.position.y != null) {
                        if (typeof data.position.y !== 'number') {
                           throw new TypeError(
                              `Invalid Resource: ${source} - If defined, property "position.y" must be of type "number"`
                           );
                        }
                     }
                     if (data.position.z != null) {
                        if (typeof data.position.z !== 'number') {
                           throw new TypeError(
                              `Invalid Resource: ${source} - If defined, property "position.z" must be of type "number"`
                           );
                        }
                     }
                  }
               ...
               if (data.texture != null) {
                  if (typeof data.texture !== 'string') {
                     throw new TypeError(
                        `Invalid Resource: ${source} - If defined, property "texture" must be of type "string"`
                     );
                  } else {
                     ...
                     if (value instanceof XTexture) {
                        ...
                        throw new ReferenceError(
                           `Invalid Resource: ${source} - If defined, property "texture" must refer to a texture file`
                        );
                     }
                  }
               }
            ...
            case 'XInput':
               if (data.keys != null) {
                  if (typeof data.keys !== 'object' || !(data.keys instanceof Array)) {
                     throw new TypeError(
                        `Invalid Resource: ${source} - If defined, property "keys" must be of type "array"`
                     );
                  } else {
                     for (const entity of data.keys.values()) {
                        if (typeof entity !== 'string') {
                           throw new TypeError(
                              `Invalid Resource: ${source} - All values in the "keys" array must be of type "string"`
                           );
                        }
                     }
                  }
            ...
            case 'XRoom':
               if (data.entities != null) {
                  if (typeof data.entities !== 'object' || !(data.entities instanceof Array)) {
                     throw new TypeError(
                        `Invalid Resource: ${source} - If defined, property "entities" must be of type "array"`
                     );
                  } else {
                     for (const [ index, entity ] of data.entities.entries()) {
                        if (typeof entity !== 'string') {
                           throw new TypeError(
                              `Invalid Resource: ${source} - All values in the "entities" array must be of type "string"`
                           );
                        } else {
                           ...
                           if (value instanceof XEntity) {
                              ...
                              throw new ReferenceError(
                                 `Invalid Resource: ${source} - The value "${entity}" at index ${index} in the "entities" array must refer to an entity file`
                              );
                           }
                        }
                     }
            ...
            case 'XTexture':
               if (typeof data.asset !== 'string') {
                  ...
                  if (value instanceof XAsset) {
                     ...
                     throw new ReferenceError(
                        `Invalid Resource: ${source} - The property "asset" must refer to an asset file`
                     );
                  }
               }
               if (data.mapping != null) {
                  if (typeof data.mapping !== 'object' || data.mapping instanceof Array) {
                     throw new TypeError(
                        `Invalid Resource: ${source} - If defined, property "mapping" must be of type "object"`
                     );
                  } else {
                     if (data.mapping.h != null) {
                        if (typeof data.mapping.h !== 'number') {
                           throw new TypeError(
                              `Invalid Resource: ${source} - If defined, property "mapping.h" must be of type "number"`
                           );
                        }
                     }
                     if (data.mapping.w != null) {
                        if (typeof data.mapping.w !== 'number') {
                           throw new TypeError(
                              `Invalid Resource: ${source} - If defined, property "mapping.w" must be of type "number"`
                           );
                        }
                     }
                     if (data.mapping.x != null) {
                        if (typeof data.mapping.x !== 'number') {
                           throw new TypeError(
                              `Invalid Resource: ${source} - If defined, property "mapping.x" must be of type "number"`
                           );
                        }
                     }
                     if (data.mapping.y != null) {
                        if (typeof data.mapping.y !== 'number') {
                           throw new TypeError(
                              `Invalid Resource: ${source} - If defined, property "mapping.y" must be of type "number"`
                           );
                        }
                     }
                  }
